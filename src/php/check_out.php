...
if(isset($data)){
	...
	if ($is_token_expired || !$is_signature_valid) {
        ...
        $query = "SELECT `user_id` FROM `favorites` WHERE `user_id` = '$user_id'";
        $result = mysqli_query($conn, $query);
        if (mysqli_num_rows($result) == 0) {
            $response = ['success' => false, 'message' => 'Do not have anything in the cart to check-out', 'reason' => 'do not have'];
            echo json_encode($response);
            exit;
        }else{
            // Start a transaction
            $conn->begin_transaction();
            $insert_query = "INSERT INTO orders (`user_id`, model, ordered_date) SELECT `user_id`, model, '$date' FROM favorites WHERE `user_id` = '$user_id'";
            $result1 = mysqli_query($conn, $insert_query);

            // Delete data from favourites table
            $delete_query = "DELETE FROM favorites WHERE `user_id` = '$user_id'";
            $result2 = mysqli_query($conn, $delete_query);

            // Commit the transaction
            if ($conn->commit()){
                $response = ['success' => true, 'message' => 'Ordered Successfully', 'data' => 'no data'];
                echo json_encode($response);
            } 
        }
    }
