...
if(isset($data)){
	...
	if ($is_token_expired || !$is_signature_valid) {
        ...
        $query = "SELECT COUNT(*) AS carnum FROM cars WHERE brand = 'lamborghini'";
        $result = mysqli_query($conn, $query);

        if ($result) {
            $carnum = mysqli_fetch_assoc($result)['carnum'];
            $response = array('success' => false, 'message' => 'got the number of lambo', 'data' => $carnum);
            echo json_encode($response);
        }else{
