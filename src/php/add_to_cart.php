...
if(isset($data)){
	...
	if ($is_token_expired || !$is_signature_valid) {
        ...
        if (mysqli_num_rows($result) == 1) {
            $response = ['success' => false, 'message' => 'Car Model already exists in the cart.', 'reason' => 'already have'];
            echo json_encode($response);
            exit;
        }else{
            $sql = "INSERT INTO `favorites` (`user_id`, `model`, `favorited_date`) VALUES ('$user_id', '$model', '$date')";

            if(mysqli_query($conn, $sql)){
                $response = ['success' => true, 'message' => 'Car Added successfuly to the cart!'];
                echo json_encode($response);
            }else{
